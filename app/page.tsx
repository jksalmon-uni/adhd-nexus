...
export default function Home() {
  // --- 0. DATA ISOLATION LOGIC ---
  const [isDevMode, setIsDevMode] = useState(false);
  
  // This prefix changes based on your URL to keep dev/prod data separate
  ...
  // --- 1. PERSISTENCE & AUTO-RESET LOGIC ---
  useEffect(() => {
    ...
    load("rewards", setRewards); load("theme", setTheme, (v:any)=>v);
    load("points", setPoints, parseInt); load("xp", setTotalXp, parseInt);
    load("settings-sounds", setSoundEnabled, (v:any)=>v === 'true');
    load("overwhelm", setOverwhelmMode, (v:any)=>v === 'true');
    load("garden", setGardenPlants); load("last-plant", setLastPlantedDate, (v:any)=>v);
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      save("rewards", rewards); save("theme", theme, (v:any)=>v);
      save("points", points, (v:any)=>v.toString()); save("xp", totalXp, (v:any)=>v.toString());
      save("daily-points", dailyPointsEarned, (v:any)=>v.toString());
      save("settings-sounds", soundEnabled.toString(), (v:any)=>v);
      save("overwhelm", overwhelmMode.toString(), (v:any)=>v);
      save("rituals", rituals); save("garden", gardenPlants); save("last-plant", lastPlantedDate, (v:any)=>v);
    }
  ...
  // --- AUDIO EFFECT FOR BODY DOUBLING ---
  useEffect(() => {
  ...
  const completeTask = (taskId: string) => {
    ...
    if (dailyPointsEarned < DAILY_CAP) {
      ...
      setPoints(p => p + allowed); setDailyPointsEarned(d => d + allowed);
    }
  ...
  const startFocusTimer = (t: Task, overrideMins?: number) => { 
    ...
    const tempTask = { ...t, duration: finalMins };
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center font-sans transition-all duration-700 ${colorMap.bg} relative overflow-x-hidden`}>
          ...
          <motion.div key={activeTab + overwhelmMode} initial={{ opacity: 0, x: 5 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -5 }} transition={{ duration: 0.2 }}>
            ...
            {activeTab === "focus" && !overwhelmMode && (
              <div className="space-y-6">
                {rituals.some(r => !r.completed) && (
                    <div className={`p-5 rounded-[32px] border ${colorMap.card} shadow-sm`}>
                  ...
                  {tasks.filter(t => t.date === new Date().toISOString().split('T')[0]).map((t, index) => (
                    <motion.div layout key={`task-${t.id}-${index}`} className={`rounded-[32px] border-2 transition-all ${colorMap.card} ${getTaskStyles(t.priority)}`}>
                      ...
                      <div onMouseDown={() => startHolding(t.id)} onMouseUp={stopHolding} onTouchStart={() => startHolding(t.id)} onTouchEnd={stopHolding} className="w-full flex justify-between items-center p-6 text-left relative group select-none cursor-pointer">
                        <div className="flex flex-col gap-1">
                            <span className={`font-bold flex items-center gap-3 ${t.priority === 'urgent' ? 'text-lg text-rose-400' : 'text-sm'}`}><div className={`w-3.5 h-3.5 rounded-full border-2 flex-shrink-0 ${holdingTaskId === t.id ? 'border-emerald-500' : t.priority === 'urgent' ? 'bg-rose-500 border-rose-500' : t.priority === 'high' ? 'border-amber-500' : colorMap.taskBullet}`}/> {t.text}</span>
                        </div>
            ...
            {activeTab === "rewards" && !overwhelmMode && (
              <div className="space-y-4 pt-4">
                ...
                <form onSubmit={(e) => { e.preventDefault(); if(!newRewardTitle || !newRewardCost || Number(newRewardCost) <= 0) return; setRewards([...rewards, { title: newRewardTitle, cost: Number(newRewardCost), id: Date.now().toString() }]); setNewRewardTitle(""); setNewRewardCost(""); }} className={`p-3 rounded-[32px] mt-8 mb-6 flex items-center gap-2 ${colorMap.card} overflow-hidden`}>
                  ...
                  <input type="number" value={newRewardCost} onChange={e => setNewRewardCost(e.target.value)} placeholder="Cost" min="1" className={`w-24 px-3 py-3 rounded-2xl text-sm ${colorMap.input}`} />
                  <button type="submit" className="bg-purple-600 w-12 h-12 flex flex-shrink-0 items-center justify-center rounded-2xl font-bold text-white"><Plus size={20}/></button>
                </form>
                {rewards.map((r, i) => (
              ...
              {!isVentMode ? (
                <>
                  ...
                  <form onSubmit={handleAddDump} className="flex gap-3 mb-10">
                    <input value={inputDump} onChange={e => setInputDump(e.target.value)} placeholder="Get it out of your head..." className={`flex-1 border-2 rounded-[24px] px-6 py-4 outline-none focus:ring-4 focus:ring-amber-500/20 ${colorMap.dumpCard} ${isDark ? 'text-white' : 'text-zinc-900'}`} />
                    <button type="submit" className={`text-white px-8 flex items-center justify-center rounded-[24px] font-bold text-2xl ${overwhelmMode ? 'bg-blue-600' : 'bg-zinc-900'}`}><Plus /></button>
                  </form>
                ...
                <div className="animate-in fade-in flex flex-col items-center">
                    ...
                    <textarea autoFocus value={inputDump} onChange={e => setInputDump(e.target.value)} placeholder="Type fast! Don't overthink it..." className={`w-full h-48 border-2 rounded-[24px] p-6 outline-none resize-none ${colorMap.dumpCard} ${isDark ? 'text-white' : 'text-zinc-900'}`} />
      ...
      <AnimatePresence>
        {isSettingsOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className={`w-full max-w-sm rounded-[48px] p-10 border shadow-2xl ${colorMap.card}`}>
              <div className="flex justify-between items-center mb-8"><h2 className="text-3xl font-black">Control</h2><button onClick={() => setIsSettingsOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800"><X size={18}/></button></div>
              <div className="space-y-4">
                <button onClick={() => setTheme(isDark ? 'light' : 'dark')} className={`w-full p-6 rounded-3xl text-left border-2 font-bold ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-slate-50 border-slate-200'}`}>{isDark ? '☀️ Light Mode' : '🌙 Dark Mode'}</button>
                <button onClick={() => setSoundEnabled(!soundEnabled)} className={`w-full p-6 rounded-3xl text-left border-2 font-bold ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-slate-50 border-slate-200'}`}>{soundEnabled ? '🔊 Sounds On' : '🔇 Sounds Muted'}</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      {/* --- THE WIN LOG (VAULT) MODAL --- */}
      <AnimatePresence>
        {isVaultOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className={`w-full max-w-sm rounded-[48px] p-8 ${colorMap.card} max-h-[80vh] flex flex-col`}>
                <div className="flex justify-between items-center mb-6">
                    <div>
                    ...
                    <button onClick={() => setIsVaultOpen(false)} className="w-10 h-10 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 rounded-full shrink-0"><X size={18}/></button>
                </div>
                <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1">
                    {completedTasks.length === 0 ? <p className="opacity-40 italic text-center py-10">Empty for now. Go get a win.</p> : completedTasks.map((t, idx) => (
      ...
      <AnimatePresence>
        {mysteryPrize && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[70] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.5, opacity: 0, rotate: -10 }} animate={{ scale: 1, opacity: 1, rotate: 0 }} className={`w-full max-w-sm rounded-[48px] p-10 bg-purple-600 text-white text-center shadow-2xl`}>
                ...
                <h3 className="text-4xl font-black mb-8 text-amber-300 drop-shadow-md">{mysteryPrize}</h3>
                <button onClick={() => setMysteryPrize(null)} className="w-full p-4 rounded-3xl bg-white text-purple-600 font-black text-lg shadow-lg active:scale-95 transition-all">Claim Reward</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
